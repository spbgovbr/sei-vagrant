...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  config.vm.provision "docker" do |docker|    
    ...
    docker.run "sei", image: "processoeletronico/sei:latest", 
      daemonize: true, 
      args: "-p 80:80 --link db:db --link solr:solr --link memcached:memcached --link jod:jod --link smtp:smtp -v /mnt/sei/src:/opt"
  end
  ...
  config.vm.provision "shell", run: "always", inline: "docker restart db && docker restart jod && docker restart solr && docker restart memcached && docker restart smtp && docker restart sei"
end
